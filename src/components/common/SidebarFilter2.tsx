...
function useTabSlide() {
  useEffect(() => {
    ...
    const tabItems = Array.from(tabSlide.querySelectorAll("li.nav-tab-item"));
    ...
    return () => {
      tabItems.forEach((item) => {
        item.removeEventListener("click", handleTabClick as EventListener);
      });
...
export default function SidebarFilter2({
  ...
  const { city, bedrooms, price, bathrooms, minSize, maxSize, garages } =
    allProps;
  useTabSlide();
